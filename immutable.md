...
对于还无法容器化，只能运行在虚拟机里的传统应用，我们该如何选择最优的部署方案呢？
...
相对于可变的服务器部署模式，不可变的服务器部署模式要求每次做部署变更时，不再更新或升级现存的服务器。
...
整个过程中，出现任何错误，我们将直接退出来，待问题解决之后，根据以上过程重新打包镜像。如果一切顺利，虚拟机创建成功，再将线上环境流量切换到该新虚拟机上，随后销毁掉的虚拟机。这样就完成一次部署变更。

如果线上流量较高，需要横向扩展虚拟机数量时，只需将上述已经打包好的应用镜像部署成虚拟机，加入到线上集群即可。整个响应过程十分迅速且可靠。
